...
def extract_and_validate_token(authorization: Optional[str]) -> Optional[str]:
    """
    ...
    # Validate token format: should be URL-safe base64, minimum 32 chars
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> dict:
    ...
    token = extract_and_validate_token(authorization)
...
# Allowed image MIME types (secure whitelist)
ALLOWED_IMAGE_TYPES = {
    'image/jpeg': '.jpg',
    'image/png': '.png',
    'image/webp': '.webp'
...
# Blocked image types (security risk - can contain scripts)
BLOCKED_IMAGE_TYPES = {
...
def validate_image_base64(image_data: str) -> tuple[bool, str, str]:
    ...
    Returns: (is_valid, error_message, validated_base64)
    
    Security checks:
    1. Valid base64 encoding
    2. Allowed MIME type (JPEG, PNG, WebP only)
    3. Actual decoded content matches claimed type
    4. No embedded scripts (especially in SVG)
    """
    import base64
    ...
    if image_data.startswith('data:'):
        ...
        # No data URI prefix - will validate after decode
        mime_type = None
    
    # Validate base64 characters
    b64_clean = re.sub(r'\s', '', b64_data)  # Remove all whitespace
    if not re.match(r'^[A-Za-z0-9+/=]*$', b64_clean):
        return False, "Invalid base64 encoding", ""
    
    # Decode and validate actual content
    try:
    ...
    # If client claimed a type, verify it matches (prevents type confusion attacks)
    ...
    # Additional SVG sanitization check - scan for script tags
    if actual_mime.lower() == 'image/svg+xml':
        # This should be blocked above, but double-check
        try:
            content = image_bytes.decode('utf-8', errors='ignore').lower()
            if '<script' in content or 'javascript:' in content or 'onerror=' in content:
                return False, "SVG contains potentially malicious content", ""
        except:
            pass
...
def detect_image_type(data: bytes) -> Optional[str]:
    ...
    Returns MIME type string or None if unknown.
    """
    if len(data) < 4:
        return None
...
async def upload_dog_photo(dog_id: str, photo_data: PhotoUpload, current_user: dict = Depends(get_current_user)):
    ...
    Security: Only JPEG, PNG, and WebP images are accepted.
    SVG and other formats are blocked to prevent XSS attacks.
    """
    dog = await db.dogs.find_one({"id": dog_id, "owner_id": current_user["id"]})
    if not dog:
        raise HTTPException(status_code=404, detail="Dog not found")
    ...
    # Update dog's avatar_url with validated base64 image
    await db.dogs.update_one(
        {"id": dog_id},
        {"$set": {"avatar_url": validated_base64, "updated_at": datetime.utcnow()}}
    )
...
@limiter.limit("30/minute")  # Rate limit for premium feature
...
@limiter.limit("5/minute")  # Strict limit for expensive AI generation
...
@limiter.limit("1/minute")  # Strict rate limit for admin operations
...
@limiter.limit("5/minute")  # Rate limit for admin operations
...
@limiter.limit("10/minute")  # Rate limit for admin operations
...
@limiter.limit("1/minute")  # Very strict rate limit for destructive operation
